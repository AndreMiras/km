...
As in
[midicase comment](https://github.com/vincentbernat/snimpy/issues/21#issuecomment-73227857),
cffi dependent packages can be cross compiled as follow:
...
This is the arch we use for cross compiling, but it could be something else,
e.g. `armhf`.
...
You may want to deploy libpython, but this is not requred for compiling later
steps:
...
This steps will install `libffi-dev` and dependencies for the host, then it will
build and install it for the target platform.
